...
class Regiment:
	...
	var shock_mult := 1.0        # Methodical panic resistance
...
func setup_from_rosters(roster_a: Array, roster_b: Array, lead_bonus_a: int, lead_bonus_b: int, side_names: Array,
	...
	for side in 2:
		...
		for i in n:
			...
			r.shock_mult = maxf(0.1, 1.0 - panic_resist)
...
func combat_tick() -> void:
	...
	for r: Regiment in regiments:
		...
		r.shock = maxf(0.0, r.shock - r.leadership * LEADERSHIP_REGEN)
		if r.soldiers <= 0:
			r.fled = true
			event.emit("%s is wiped out!" % r.label)
		elif not r.routed and r.morale() <= 0.0:
			r.routed = true
...
func _ranged_tick(dmg: Dictionary) -> void:
	...
	for r: Regiment in regiments:
		...
		if t == null or r.pos.distance_to(t.pos) > r.rng_range:
			continue
		r.ammo -= 1
		var block: float = t.shield * SHIELD_ARC_FACTOR[_arc(r, t)]
